...
try {
    ...
    foreach ($case in @(
        ...
        @{ Lines = @($header, ($main -replace (Name-Hex 'game.exe'), 'c080')); Error = 'UTF-8' },
        @{ Lines = @($header, ($main -replace (Name-Hex 'game.exe'), 'eda080')); Error = 'UTF-8' },
        @{ Lines = @($header, ($main -replace (Name-Hex 'game.exe'), 'f4908080')); Error = 'UTF-8' },
