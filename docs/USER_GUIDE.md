...
**2. Open your browser** to the frontend URL printed by `./scripts/dev.sh`.
...
- **TLB is modeled directionally** - Page-walk and TLB behavior is modeled, but
  profile confidence depends on calibration evidence.
- **Bandwidth is estimated** - Memory bandwidth and MLP are directional model
  inputs, not measured wall-clock throughput.
...
- **Issues**: Report bugs at [GitHub Issues](https://github.com/AveryClapp/Cache-Explorer/issues)
