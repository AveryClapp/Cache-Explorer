...
Newer results also include an estimated execution summary:

```
Primary bottleneck: memory
Estimated cycles: 651742
Confidence: medium
Top source: examples/conv2d_kernel.c:36
```

**Primary bottleneck** points at the dominant estimated stall source:
- **memory**: data-cache misses and exposed memory latency dominate.
- **branch**: branch mispredictions dominate.
- **frontend**: instruction fetch/cache stalls dominate.
- **balanced**: no single modeled stall source dominates.

**Estimated cycles** is a directional pipeline estimate, not a replacement for a cycle-accurate CPU model. Use it to compare versions of the same kernel under the same settings.

**Top source** is the source line with the largest attributed share of the primary bottleneck. It is also emitted in JSON as `summary.topSource` and repeated in `sourceAnnotations`.

## Evidence & Fidelity

The Evidence & Fidelity panel explains how reproducible a result is and where
the model boundary sits.

- **Hardware** names the selected profile and profile ID.
- **Fidelity** shows whether the trace was full or sampled, and whether fast
  mode, loop caching, or prefetch modeling changed the analysis path.
- **Model** combines the hardware profile confidence with validation
  confidence.
- **Executor** shows whether the run came from the sandbox, direct local
  execution, or a cached result.
- **Compiler** and **Simulator** record the toolchain used for the result.
- **Local Repro Command** is the shortest command that should recreate the same
  analysis locally.

The hardware model contract separates fields into modeled, estimated,
metadata-only, and unsupported buckets. Treat modeled and calibrated fields as
direct simulation inputs. Treat estimated fields as useful directional
assumptions, especially when comparing variants under the same profile.

## Comparing Hardware Profiles

Use comparison mode to see how the same trace behaves under different modeled profiles:

```bash
./backend/scripts/cache-explore compare examples/conv2d_kernel.c -O2 --configs educational,intel14,zen4,m3
```

The command compiles and runs the instrumented program once, then replays the captured trace through each profile. The human output starts with a Hardware Summary table and follows with cache hit rates; `--json` keeps the full per-profile results under `configs` and adds a compact top-level `summary` array.
